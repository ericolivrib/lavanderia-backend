...
public class SchedulingMapper {
    ...
    public <T> T mapToDataTransferObject(Scheduling scheduling, Class<T> targetClass) {
        if (targetClass.equals(CreateSchedulingResponseDTO.class)) {
            ...
            var dto = new CreateSchedulingResponseDTO(scheduling.getId(), scheduling.getUser().getRegistration(), dateTime.toLocalDate(), dateTime.toLocalTime(), scheduling.getStatus());
